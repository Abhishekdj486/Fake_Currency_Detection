...
def register(request):
    if request.method == "POST":
        ...
        # Create a new user
        myuser = User.objects.create_user(username=uname, password=password, email=email,first_name=fname, last_name=lname)
        myuser.save()
...
def FakeCurrencyDetection(request):
    if request.method == 'GET':
        return render(request, 'FakeCurrencyDetection.html', {})
...
def TrainML(request):
    if request.method == 'GET':
        return render(request, 'TrainML.html', {})
    
def TrainML(request):
    if request.method == 'GET':
        if 'dataset' not in globals():
            error_message = "Please upload the dataset before preprocessing."
            context = {'error_message': error_message}
            return render(request, 'UserScreen.html', context)
...
def PreprocessDataset(request):
    if request.method == 'GET':
        return render(request, 'PreprocessDataset.html', {})
...
def SignupAction(request):
    if request.method == 'POST':
        ...
        status = 'none'
        con = pymysql.connect(host='127.0.0.1',port = 8000,user = 'root', password = 'root', database = 'fakecurrency',charset='utf8')
        with con:
            cur = con.cursor()
            ...
            for row in rows:
                if row[0] == user:
                    ...
                    break
        if status == 'none':
            db_connection = pymysql.connect(host='127.0.0.1',port = 3306,user = 'root', password = 'root', database = 'fakecurrency',charset='utf8')
